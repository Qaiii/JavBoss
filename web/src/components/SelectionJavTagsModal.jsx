...
function SelectedTag({ tag, disabled, onRemove }) {
  return (
    <span className="inline-flex min-w-0 items-center gap-1 rounded-full bg-app-surface-2 px-2 py-1 text-sm text-app-text">
...
export default function SelectionJavTagsModal({
  ...
  return (
    <AppModal
      ...
      contentClassName="flex max-h-[85vh] w-full max-w-lg flex-col rounded-2xl bg-app-surface shadow-2xl"
      onClose={onClose}
      zIndex={1600}
    >
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 pb-5">
        <div>
          <div className="mb-2 grid grid-cols-[minmax(0,1fr)_auto] gap-3 px-2 text-xs font-medium text-app-muted">
            <span>{zh('文件名', 'Filename')}</span>
            <span>{zh('番号', 'Code')}</span>
          </div>
          <ul className="max-h-48 divide-y overflow-y-auto rounded-md border border-app-border bg-app-surface-2">
            {list.map((item) => (
              <li
                key={item.id}
                className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-3 py-2 text-sm"
              >
                <span className="truncate text-app-text" title={item.label}>
                  {item.label}
                </span>
                <span className="font-medium text-app-text">{item.jav_code || '-'}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          <div className="text-sm font-medium text-app-text">{zh('自定义标签', 'Custom tags')}</div>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            <button
              type="button"
              className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-app-border text-app-text hover:bg-app-surface-2 disabled:cursor-not-allowed disabled:opacity-60"
