...
export default function JavSelectionTagsModal({
  ...
  return (
    <AppModal
      ...
      contentClassName="flex max-h-[85vh] w-full max-w-sm flex-col rounded-2xl bg-app-surface p-4 shadow-xl"
      onClose={onClose}
      zIndex={1600}
    >
      ...
      <div className="min-h-0 flex-1 space-y-1 overflow-y-auto rounded border p-2">
        {list.length === 0 ? (
          <p className="px-2 py-1 text-sm text-app-muted">
          ...
          list.map((tag) => (
            <label
              key={tag.id}
              className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 hover:bg-app-surface-2"
            >
              <input
                type="checkbox"
                ...
                aria-label={tag.name}
              />
              <span className="min-w-0 flex-1 break-words text-sm text-app-text">{tag.name}</span>
