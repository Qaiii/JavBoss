...
export default function JavIdolCoverModal({
  ...
  return (
    <ModalShell
      open={open}
      onClose={onClose}
      title={getIdolDisplayName(item, preferChineseName)}
      subtitle={zh('封面', 'Cover')}
      closeLabel={zh('关闭', 'Close')}
      backdropClassName="bg-slate-950/70 backdrop-blur-sm px-4 py-6"
      panelClassName="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-lg bg-white shadow-2xl"
      headerClassName="flex items-center justify-between border-b border-slate-200 px-4 py-3"
      titleClassName="truncate text-base font-semibold text-slate-950"
