...
func listJavIdols(c *gin.Context) {
	...
	items, total, err := dbpkg.ListJavIdols(c.Request.Context(), search, sort, limit, offset, nil, parseClosedSubdirectories(c.Query("closed_subdirs")), parseDirectorySubpaths(c.Query("directory_subpaths")), favoriteGroupID, filters)
	if err != nil {
